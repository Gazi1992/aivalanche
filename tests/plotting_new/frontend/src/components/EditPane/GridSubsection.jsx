...
const GridSubsection = ({
  gridExpanded,
  setGridExpanded,
  hasGrid = true,  // Default to true for backward compatibility
  hasPlotArea = true,  // Default to true for backward compatibility
  xGridVisible,
  setXGridVisible,
  yGridVisible,
  setYGridVisible,
    ...
    <div style={subsectionStyle}>
      ...
      {gridExpanded && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {/* Grid controls - only show if plot has grid capability */}
          {hasGrid && (
            <>
              {/* X-axis Grid Lines */}
          ...
          <div style={{
            display: 'flex',
            alignItems: 'center',
            justifyContent: hasGrid ? 'space-between' : 'flex-start',
            width: '100%',
            marginTop: hasGrid ? '8px' : '0',
            gap: '20px'
          }}>
            {/* Grid - only show if plot has grid */}
            {hasGrid && (
              <>
            ...
            {/* Plot Area - only show if plot has distinct plot area */}
            {hasPlotArea && (
              <>
                {/* Vertical Divider - only between Figure and Plot Area */}
                <div style={{
                  width: '1px',
                  height: '24px',
                  backgroundColor: 'var(--border-color)',
