...
try:
    from themes import AVAILABLE_THEMES, DEFAULT_THEME_KEY
    VALID_THEME_CONFIG_KEYS = list(AVAILABLE_THEMES.keys())
    if DEFAULT_THEME_KEY not in VALID_THEME_CONFIG_KEYS:
        logger.warning(f"DEFAULT_THEME_KEY '{DEFAULT_THEME_KEY}' from themes.py "
                       f"is not in its own AVAILABLE_THEMES. Using 'light' as schema default.")
        SCHEMA_DEFAULT_THEME = "light"
        if "light" not in VALID_THEME_CONFIG_KEYS:
            VALID_THEME_CONFIG_KEYS.append("light")
    else:
        SCHEMA_DEFAULT_THEME = DEFAULT_THEME_KEY

except ImportError:
    logger.warning("Could not import from themes.py in config_schema.py. "
                   "Theme validation will use a basic default list: ['light', 'dark'].")
    VALID_THEME_CONFIG_KEYS = ["light", "dark"]
    SCHEMA_DEFAULT_THEME = "light"
...
class SchemaRegistry:
    ...
    BAR_PLOT_SCHEMA: Dict[str, ConfigField] = {
        ...
        "stack_group": ConfigField(
            field_type=(str, type(None)),
            default=None,
            description="Identifier for stacked bar grouping. Items with same non-null value will be stacked atop each other."
        ),
        "group_index": ConfigField(
            field_type=(int, type(None)),
            default=0,
            validator=is_non_negative,
            description="Index of the series within a grouped bar chart (0-based)."
        ),
        "group_count": ConfigField(
            field_type=(int, type(None)),
            default=1,
            validator=is_positive,
            description="Total number of series in the group. Use the same value for all grouped bar items."
        ),
    }
