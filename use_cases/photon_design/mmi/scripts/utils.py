#%% Imports
import os, json, matplotlib.pyplot as plt, numpy as np, pandas as pd
...
def dict_2_csv(data = None, file_path = None):
...
def normalize_dict_with_df(input_dict, df):
    """
    Normalize dictionary values based on min/max values from a dataframe.

    Parameters:
    input_dict (dict): Dictionary with parameter values
    df (DataFrame): DataFrame with min/max values and scale information
    ...
    for key, value in input_dict.items():
        # Find the row in dataframe that matches the key
        row = df[df['name'] == key]

        if len(row) == 0:
            continue  # Skip if parameter not found in dataframe
...
def simulate_multiple_parameters(parameters: list[dict] = None, **kwargs):
    ...
    if simulator is not None and parameters is not None:
        results = simulator.simulate_multiple_parameters(parameters, **kwargs)
...
def calculate_metric(sim_results):
    if sim_results['power'] is not None:
        metric = -sim_results['power']
    else:
        metric = sim_results['penalty']
    return metric

def simulate_multiple_widths(parameters: list[dict] = None, **kwargs):
    simulator = kwargs['simulator']
    parameters_lengths_path = kwargs['parameters_lengths_path']
    seed = kwargs['seed'] if 'seed' in kwargs.keys() else None
    results = []
    if simulator is not None and parameters is not None and parameters_lengths_path is not None:
        for p in parameters:
            if simulator.is_valid_geometry(p):
                lengths = optimize_lengths_de(widths = p,
                                              parameters_path = parameters_lengths_path,
                                              simulator = simulator,
                                              seed = seed)

                # lengths = optimize_lengths_nm(widths = p,
                #                               parameters_path = parameters_lengths_path,
                #                               simulator = simulator,
                #                               seed = seed)

                all_params = {**p, **lengths}
                res = simulator.simulate_parameters(all_params)
            else:
                res = {'parameters': p,
                       'power': None,
                       'penalty': simulator.calculate_geometry_penalty(p)}
            results.append(res)
    metrics = [calculate_metric(r) for r in tqdm(results, desc = "Calculating metrics")]
    responses = {'data': results, 'metrics': metrics}
    return responses

def simulate_multiple_lengths(parameters: list[dict] = None, **kwargs):
    simulator = kwargs['simulator']
    widths = kwargs['widths']
    if simulator is not None and parameters is not None and widths is not None:
        combined_parameters = [{**item, **widths} for item in parameters]
        results = simulator.simulate_multiple_parameters(combined_parameters, fetch_z_field = False)
    metrics = [calculate_metric(r) for r in tqdm(results, desc = "Calculating metrics")]
    responses = {'data': results, 'metrics': metrics}
    return responses

def optimize_lengths_de(widths: dict = None, parameters_path: str = None, simulator = None, seed = None,
                        pop_size = 10, max_iterations = 100, max_iter_without_improvement = 10):
    parameters = read_parameters(parameters_path)
    eval_func_args = {'simulator': simulator, 'widths': widths}
    # Initialize optimizer
    diff_evolution = Differential_evolution(seed = seed,
                                            parameters = parameters,
                                            eval_func = simulate_multiple_lengths,
    ...
    return diff_evolution.get_best_parameters()

def optimize_lengths_nm(widths: dict = None, parameters_path: str = None, simulator = None, seed = None,
                        pop_size = 10, max_iterations = 50, max_iter_without_improvement = 10):
    parameters = read_parameters(parameters_path)
    eval_func_args = {'simulator': simulator, 'widths': widths}
    # Initialize optimizer
...
def plot_z_field_and_profile(z_field = None, profile = None, ax = None, path = None):
    ...
    if z_field is not None or profile is not None:
        ...
        if profile is not None:
            plt.plot(profile[:,0], profile[:,1], color = 'w', linewidth = 1)
...
def plot_results_nm(sim_data, optimization_data = None, path = None):
    ...
    title_ax = figure.add_axes([0, 1-top_margin, 1, top_margin])
    title_ax.axis('off')
    title_ax.text(0.5, 0.7, 'Design of an optimal splitter',
                 horizontalalignment='center',
                 verticalalignment='center',
                 fontsize=16,
                 fontweight='bold')
