...
def plot_results(sim_data, optimization_data = None, path = None):
    ...
    width = monitor.width / dpi * 1.1 * 0.7 # Convert pixels to inches (approximate)
    height = monitor.height / dpi * 1 * 0.7
    ...
    title_ax.axis('off')
    title_ax.text(0.5, 0.7, 'Transistor calibration',
    ...
    loss_height = available_height - ver_space - fit_height
    param_height = loss_height
    ...
    param_x = loss_x + loss_width + hor_space_2
    param_y = bottom_margin

    # Add the fit plots
    group_ids = [1, 10, 23, 25]
    nr_fit_plots = len(group_ids)
    fit_width = (all_fit_width - (nr_fit_plots - 1) * hor_space_1) / nr_fit_plots
    for i, id in enumerate(group_ids):
        x = fit_x + i * (fit_width + hor_space_1)
        ax = figure.add_axes([x, fit_y, fit_width, fit_height])
        plot_fit(sim_data[sim_data['group_id'] == id], ax = ax)
