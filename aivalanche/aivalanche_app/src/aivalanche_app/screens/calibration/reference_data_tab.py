...
class reference_data_tab(QSplitter):
    ...
    def on_import_new_ref_data_file(self, file_path: str = None):
        if file_path is not None:
            ...
            if file_valid:
                new_file_path = Path.joinpath(self.store.active_project_common_reference_data_directory, Path(file_path).name)
                if Path.exists(new_file_path):
                    if file_path in self.store.available_reference_data['original_path'].tolist():
                        file_exists = True
                    else:
                        new_file_path = find_max_suffix(self.store.active_project_common_reference_data_directory, Path(file_path).name)
                if file_exists:
